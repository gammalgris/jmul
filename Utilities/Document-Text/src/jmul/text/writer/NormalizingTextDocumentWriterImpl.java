...
/**
 * An implementation of a document writer which normalizes the document content before writing.
 *
 * @author Kristian Kutin
 */
public class NormalizingTextDocumentWriterImpl implements TextDocumentWriter {

    /**
     * The default constructor.
     */
    public NormalizingTextDocumentWriterImpl() {
    ...
    public void writeTo(File aFile, TextDocument aDocument) throws IOException {
        ...
        try (OutputStreamWriter osw = new OutputStreamWriter(new FileOutputStream(aFile), charset)) {
            ...
            for (String line : aDocument.getContent().getContent()) {
                ...
                String normalizedLine = TextNormalizationHelper.toNormalizedJavaString(line);

                String nextLine;
                if (firstLine) {

                    firstLine = false;
                    nextLine = normalizedLine;

                } else {

                    nextLine = TextHelper.concatenateStrings(lineSeparator, normalizedLine);
                }
